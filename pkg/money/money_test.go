...
func TestMoney_Add(t *testing.T) {
	...
	b := mustMoney(t, "5.50", "USD")
	got, err := a.Add(b)
	if err != nil {
		t.Fatalf("Add: %v", err)
	}
	want := mustMoney(t, "15.50", "USD")
	if !got.Equal(want) {
...
func TestMoney_Sub(t *testing.T) {
	a := mustMoney(t, "10.00", "USD")
	b := mustMoney(t, "3.25", "USD")
	got, err := a.Sub(b)
	if err != nil {
		t.Fatalf("Sub: %v", err)
	}
	want := mustMoney(t, "6.75", "USD")
	if !got.Equal(want) {
...
func TestMoney_Equal(t *testing.T) {
	...
	otherAmt := mustMoney(t, "10.51", "USD")
	if a.Equal(otherAmt) {
...
func TestMoney_String(t *testing.T) {
	m := mustMoney(t, "1234.5", "USD")
	if got := m.String(); got != "1234.50 USD" {
		t.Errorf("String() = %q, want %q", got, "1234.50 USD")
	}
}
