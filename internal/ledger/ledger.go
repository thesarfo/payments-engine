...
type Repository interface {
	...
	InsertJournalEntryTx(ctx context.Context, tx pgx.Tx, entry JournalEntry) (uuid.UUID, error)
}
...
// PostJournalEntryTx enforces the same double-entry invariants as PostJournalEntry
// but participates in a caller-supplied transaction. The caller owns commit/rollback.
func (l *Ledger) PostJournalEntryTx(ctx context.Context, tx pgx.Tx, entry JournalEntry) (uuid.UUID, error) {
	totalDebits := decimal.Zero
	totalCredits := decimal.Zero
	...
	return l.repo.InsertJournalEntryTx(ctx, tx, entry)
}
